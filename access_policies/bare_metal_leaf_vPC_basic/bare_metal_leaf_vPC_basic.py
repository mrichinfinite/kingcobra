# Basic access policies for connecting a bare metal (physical) server/host to ACI via vPC. Change the variables as needed for your environment.
...
'''
The CDP interface policy, which is primarily used to obtain protocol addresses of neighboring devices and discover the platform of those devices. 
...
'''
The LLDP interface policy, which defines a common configuration that will apply to one or more LLDP interfaces. 
...
# Alias:
lldpIfPol_nameAlias = ''
# Receive admin state: enabled or disabled?
lldpIfPol_adminRxSt = 'enabled'
# Transmit admin state: enabled or disabled?
...
'''
The PortChannel policy enables you to bundle several physical ports together to form a single port channel. 
LACP enables a node to negotiate an automatic bundling of links by sending LACP packets to the peer node.
'''
# Name: The interface policy name.
...
# Minimum number of links? Choose 1-16.
lacpLagPol_minLinks = '1'
# Maximum number of links? Choose 1-16.
...
# Node Block (infraNodeBlk) (child of Leaf Selector)
'''
...
# Type: The host port selector type.
...
'''
# tDn: Select or create an interface policy group to associate to the Access Port selector.
infraRsAccBaseGrp_tDn = 'uni/infra/funcprof/accbundle-vPC'
# Annotation:
infraRsAccBaseGrp_annotation = ''
# Fex Id: The interface policy group FEX ID.
...
# Access Port Block (infraPortBlk) (child of Interface Selector)
'''
...
# lagT: The bundled ports group link aggregation type: port channel vs virtual port channel.
...
# Starting VLAN in range:
fvnsEncapBlk_from = 'vlan-1'
# Ending VLAN in range:
fvnsEncapBlk_to = 'vlan-4094'
# Allocation Mode: static, dynamic or inherit?
...
# Pairing Type: explicit, consecutive or reciprocal?
...
# ID: Select the node identifier to be added to the vPC pair in the protection group.
