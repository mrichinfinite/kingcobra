# Basic tenant policies for connecting a bare metal (physical) endpoint to ACI via leaf trunk port. Change the variables as needed for your environment. Tested and verified on ACI software version 5.2(3e).
...
# Static Path (fvRsPathAtt)
'''
A source relation to an abstraction of a path endpoint.
'''
# Annotation: An annotation for enabling clients to add arbitrary key:value pairs of metadata to an object.
fvRsPathAtt_annotation = ''
# Description: Specifies a description of the policy definition.
fvRsPathAtt_descr = ''
# User Domain: The AAA domain to which the user belongs.
fvRsPathAtt_userdom = 'all'
# Encap: The encapsulation method (VLAN) of the path.
fvRsPathAtt_encap = 'vlan-2'
# Deployment Immediacy (The deployment immediacy preference of this path association.): immediate or lazy? (lazy = On Demand)
fvRsPathAtt_instrImedcy = 'immediate'
# Mode (The mode of the static association with the path.): native, regular or untagged? (native = Access (802.1P), regular = Trunk, untagged = Access (Untagged))
fvRsPathAtt_mode = 'regular'
# Primary Encap: If the application EPG requires a primary VLAN, enter the primary VLAN here.
fvRsPathAtt_primaryEncap = 'unknown'
# tDn: The target name of the static path binding.
fvRsPathAtt_tDn = 'topology/pod-1/paths-101/pathep-[eth1/1]'
# Configure policy:
fvRsPathAtt = cobra.model.fv.RsPathAtt(fvAEPg, annotation=fvRsPathAtt_annotation, descr=fvRsPathAtt_descr, encap=fvRsPathAtt_encap, instrImedcy=fvRsPathAtt_instrImedcy, mode=fvRsPathAtt_mode, primaryEncap=fvRsPathAtt_primaryEncap, tDn=fvRsPathAtt_tDn, userdom=fvRsPathAtt_userdom)
...
fvRsDomAtt_tDn = 'uni/phys-Phys_Dom'
# Binding Type: dynamicBinding, ephemeral, staticBinding or none?
fvRsDomAtt_bindingType = 'none'
